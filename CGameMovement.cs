...
//
// Purpose: REIMPLEMENTATION OF THE CGAMEMOVEMENT CLASS TO UNITY C#
//
// $NoKeywords: $
//=============================================================================//

//YOU DON'T INCLUDE FILES IN C# - IT JUST DOESN'T WORK THAT WAY - WE NEED TO LOSE THIS SECTION
...
public class CGameMovement : MonoBehaviour
{
    ...
    const float CHECKSTUCK_MINTIME = 0.05f;  // Don't check again too quickly.
    const float NON_JUMP_VELOCITY = 140.0f;
    static ConVar sv_optimizedmovement = new ConVar( "sv_optimizedmovement", "1", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY );

    virtual uint PlayerSolidMask(bool brushOnly = false);   /////< returns the solid mask for the given player, so bots can have a more-restrictive set
    CBasePlayer player;
    ...
    const float TICK_INTERVAL = 1f; //CANT FIND THIS IN THE ORIGINAL SO I MADE UP A VALUE FOR NOW - JAMES

    const float CATEGORIZE_GROUND_SURFACE_INTERVAL = 0.3f;
    const float CATEGORIZE_GROUND_SURFACE_TICK_INTERVAL = ((int)(CATEGORIZE_GROUND_SURFACE_INTERVAL / TICK_INTERVAL));

    const float CHECK_STUCK_INTERVAL = 1.0f;
    const float CHECK_STUCK_TICK_INTERVAL = ((int)(CHECK_STUCK_INTERVAL / TICK_INTERVAL));

    const float CHECK_STUCK_INTERVAL_SP = 0.2f;
    const float CHECK_STUCK_TICK_INTERVAL_SP = ((int)(CHECK_STUCK_INTERVAL_SP / TICK_INTERVAL));

    const float CHECK_LADDER_INTERVAL = 0.2f;
    const float CHECK_LADDER_TICK_INTERVAL = ((int)(CHECK_LADDER_INTERVAL / TICK_INTERVAL));

    const float NUM_CROUCH_HINTS = 3;
    ...
    public CGameMovement()
    {
        ...
        //memset(m_flStuckCheckTime, 0, sizeof(m_flStuckCheckTime)); Probably don't need this...
    }
    ...
    //-----------------------------------------------------------------------------
    //public uint PlayerSolidMask(bool brushOnly)
    //{
    ...
    // Output : int
    //-----------------------------------------------------------------------------
    public int GetCheckInterval(IntervalType_t type)
    {
        int tickInterval = 1;
        switch (type)
        {
            ...
            case IntervalType_t.GROUND:
                tickInterval = (int)CATEGORIZE_GROUND_SURFACE_TICK_INTERVAL;
                break;
            case IntervalType_t.STUCK:
                ...
                else
                {
                    ...
                    {
                        tickInterval = (int)CHECK_STUCK_TICK_INTERVAL_SP;
                    }
                    else
                    {
                        tickInterval = (int)CHECK_STUCK_TICK_INTERVAL;
                    }
                }
                break;
            case IntervalType_t.LADDER:
                tickInterval = (int)CHECK_LADDER_TICK_INTERVAL;
                break;
        }
        return tickInterval;
    }
    ...
    //-----------------------------------------------------------------------------
    public bool CheckInterval(IntervalType_t type)
    {
    ...
    CBaseHandle TestPlayerPosition(Vector3 pos, int collisionGroup, trace_t pm) //THIS WHOLE FUNCTION WILL NEED ADAPTING FOR THE UNITY RAY CLASS
    {
        Ray ray;
    ...
    //-----------------------------------------------------------------------------
    void DrawDispCollPlane(CBaseTrace pTrace) //THIS WILL NEED CONVERSION FOR THE UNITY VECTOR3 CLASS
    {
        float flLength = 30.0f;

        // Create a basis, based on the impact normal.
        int nMajorAxis = 0;
        Vector3 vecBasisU, vecBasisV, vecNormal;
        ...
        Vector3 vecImpactPoint = pTrace.startpos + pTrace.fraction * (pTrace.endpos - pTrace.startpos);
        vecImpactPoint += vecNormal;

        // Generate a quad to represent the plane.
        Vector3[] vecPlanePoints = new Vector3[4];
    ...
    float ComputeConstraintSpeedFactor()
    {
        ...
        if (mv == null || mv.m_flConstraintRadius == 0.0f)
            return 1.0f;
    ...
    {
        System.Diagnostics.Debug.Assert(pMove != null);
        System.Diagnostics.Debug.Assert( pPlayer!= null);
        ...
        System.Diagnostics.Debug.Assert(player == pPlayer);
        player = pPlayer;
    ...
    void DecayPunchAngle()
    {
        if (player.m_Local.m_vecPunchAngle.LengthSqr() > 0.001 || player.m_Local.m_vecPunchAngleVel.LengthSqr() > 0.001)
        {
            ...
            player.m_Local.m_vecPunchAngle.Init(
                Mathf.Clamp(player.m_Local.m_vecPunchAngle.x, -89.0f, 89.0f),
                Mathf.Clamp(player.m_Local.m_vecPunchAngle.y, -179.0f, 179.0f),
                Mathf.Clamp(player.m_Local.m_vecPunchAngle.z, -89.0f, 89.0f));
        }
        else
        {
    ...
    //-----------------------------------------------------------------------------
    void StepMove(Vector3 vecDestination, trace_t trace)
    {
       Vector3 vecEndPos;
    ...
    bool CheckJumpButton()
    {
        ...
        player.PlayStepSound((Vector3)mv.GetAbsOrigin(), player.m_pSurfaceData, 1.0, true);
    ...
    // Output : int
    //-----------------------------------------------------------------------------
    int TryPlayerMove(Vector3 pFirstDest, trace_t pFirstTrace)
    {
       ...
       Vector3 end;
        ...
        for (bumpcount = 0; bumpcount < numbumps; bumpcount++)
        {
            ...
            if (g_bMovementOptimizations)
            {
                ...
                {
#if (PLAYER_GETTING_STUCK_TESTING)
				trace_t foo;
        ...
        else
        {
            if (forwardSpeed != 0 || rightSpeed != 0)
            {
               Vector3 velocity, perp, cross, lateral, tmp;
                ...
                //Vector3 velocity = (forward * gpGlobals.v_forward) + (right * gpGlobals.v_right);
			...
			Vector3 angleVec = perp * perpDist;
			angleVec += cross;
    ...
    // Output : const char
    //-----------------------------------------------------------------------------
#if !(_STATIC_LINKED) || (CLIENT_DLL)
    char DescribeAxis(int axis)
    {
        char sz[32];
...
const char *DescribeAxis( int axis );
    ...
    // Output : int
    //-----------------------------------------------------------------------------
    int ClipVelocity<in Vectori, out Vectoro>(Vector3 normal, float overbounce)
    {
        float backoff;
        float change;
        float angle;
        ...
        backoff = DotProduct(Vectori, normal) * overbounce;

        for (i = 0; i < 3; i++)
        {
            change = normal[i] * backoff;
            Vectoro[i] = Vectori[i] - change;
        }

        // iterate once to make sure we aren't still moving through the plane
        float adjust = DotProduct(Vectoro, normal);
        if (adjust < 0.0f)
        {
            Vectoro -= (normal * adjust);
            //		Msg( "Adjustment = %lf\n", adjust );
        }
...
#if !(_STATIC_LINKED) || (CLIENT_DLL)
   Vector3 rgv3tStuckTable[54];
#else
externVector3 rgv3tStuckTable[54];
#endif
...
extern void CreateStuckTable( void );
    ...
    // Output : int
    //-----------------------------------------------------------------------------
    int GetRandomStuckOffsets(CBasePlayer pPlayer,Vector3 offset)
    {
...
# if !DEDICATED
    ...
    int GetPointContentsCached(Vector3 point, int slot)
    {
        if (g_bMovementOptimizations)
        {
            Assert(player);
    ...
    //-----------------------------------------------------------------------------
    void TracePlayerBBoxForGround(Vector3 start,Vector3 end,Vector3 minsSrc,
                             Vector3 maxsSrc, IHandleEntity player, uint fMask,
    ...
    // the original trace hit first.
    //-----------------------------------------------------------------------------
    void TryTouchGroundInQuadrants(Vector3 start,Vector3 end, uint fMask, int collisionGroup, trace_t pm)
    {
...
#pragma warning (default : 4701)
    ...
    //-----------------------------------------------------------------------------
    void TracePlayerBBox(Vector3 start,Vector3 end, uint fMask, int collisionGroup, trace_t pm)
    {
    ...
    //-----------------------------------------------------------------------------
    void TryTouchGround(Vector3 start,Vector3 end,Vector3 mins,Vector3 maxs, uint fMask, int collisionGroup, trace_t pm)
    {
...
public class IDiffMgr
{

    public static IDiffMgr g_pDiffMgr = null;
	public virtual void StartCommand(bool bServer, int nCommandNumber)
    { }
    public virtual void AddToDiff(bool bServer, int nCommandNumber, char String )
    { }
	public virtual void Validate(bool bServer, int nCommandNumber)
    { }
};

class CDiffStr
{
    private char[] m_str = new char[128];
    ...
    public CDiffStr(char str )
	{
        Q_strncpy(m_str, str, sizeof( m_str ) );
	}

    public CDiffStr(CDiffStr src)
    {
...
class CDiffInfo
{
    ...
    //public CDiffInfo() : m_nCommandNumber( 0 ) { }
    public CDiffInfo( CDiffInfo src )
	{
    ...
    public static bool Less(CDiffInfo lhs, CDiffInfo rhs)
    {
        return lhs.m_nCommandNumber < rhs.m_nCommandNumber;
    }
    public int m_nCommandNumber;
    public CUtlVector3<CDiffStr> m_Lines;
    public bool m_bChecked;


};

class CDiffManager : IDiffMgr
{
    ...
    public override void StartCommand(bool bServer, int nCommandNumber)
    {
#if (CLIENT_DLL)
        ...
        //System.Diagnostics.Debug.Assert(CBaseEntity::IsServer());
    ...
    public override void AddToDiff(bool bServer, int nCommandNumber, char String)
    {
#if (CLIENT_DLL)
    ...
    public EMismatched IsMismatched(int svidx)
    {
    ...
    public override void Validate(bool bServer, int nCommandNumber)
    {
#if (CLIENT_DLL)
    ...
    public void SpewRecords(char prefix, int svidx)
    {
        CDiffInfo serverslot = m_Server[svidx];
	...
	private CUtlRBTree<CDiffInfo, int> m_Server;
    private CUtlRBTree<CDiffInfo, int> m_Client;
    private float m_flLastSpew;
};


//THE PROBLEM WE NOW HAVE IS THAT THERE ARE A TON OF VARIABLES AND TYPES UP TOP IN THE ORIGINAL FILE THAT CANNOT BE
//DEFINED IN THIS WAY IN C# - IT'S SIMPLY NOT POSSIBLE.
//THEY WOULD NEED TO BE CONVERTED TO BE MEMBERS OF A STATIC WRAPPER CLASS OF SOME KIND.

//THERE IS A MASSIVE AMOUNT HERE WHERE WE ARE INCLUDING ENGINE SPECIFIC LIBRARIES AND HEADERS TO CONDITIONALLY DEFINE GLOBAL VARIABLES
//AND CLASSES - MOST OF THIS CAN'T BE DONE IN C# AND WOULD BE A NIGHTMARE TO TRANSLATE - C# IS JUST FUNDAMENTALLY DIFFERENT TO C++ IN 
//WAYS THAT RENDER A LOT OF WHAT IS HAPPENING HERE IMPOSSIBLE WITHOUT REWRITING ALL THE EXTERNAL LIBRARIES

//IE. C - THERE ARE NO CLASSES, ONLY STRUCTS - STRUCTS CAN ONLY CONTAIN VARIABLES, ALL FUNCTIONS ARE GLOBAL (GENERALLY)
//    C++ - THE SAME APPLIES, BUT WE CAN HAVE CLASSES THAT CONTAIN FUNCTIONS THAT ARE LOCAL TO THEM VS GLOBAL
//    C# - THERE ARE -ONLY- CLASSES AND STRUCTS - NO GLOBAL VARIABLES OR FUNCTIONS - EVERYTHING MUST BE IN A CLASS

//IE. C - THERE ARE NO CLASSES, ONLY STRUCTS
//    C++ - THERE ARE CLASSES AND STRUCTS
//    C# - THERE ARE -ONLY- CLASSES AND STRUCTS




//THEREFORE - WE NEED A STATIC CLASS TO HOLD ALL PREVIOUSLY GLOBAL VARIABLES

public static class SourceMovementUtils
{

    const float CTEXTURESMAX = 512;     // max number of textures loaded
    const float CBTEXTURENAMEMAX = 13;          // only load first n chars of name
    ...
    const float MAX_CLIP_PLANES = 5f;
    ...
    const float tickcount = USE_PLAYER_CURRENT_COMMAND_NUMBER__INSTEAD_OF_TICKCOUNT;
    ...
    static ConVar option_duck_method("option_duck_method", "1", FCVAR_REPLICATED|FCVAR_ARCHIVE );// 0 = HOLD to duck, 1 = Duck is a toggle
...
#if STAGING_ONLY
#if CLIENT_DLL
    static ConVar debug_latch_reset_onduck( "debug_latch_reset_onduck", "1", FCVAR_CHEAT );
#endif
#endif
...
ConVar player_limit_jump_speed( "player_limit_jump_speed", "1", FCVAR_REPLICATED );
    ...
    // See shareddefs.h
    //THIS KIND OF THING WILL BE A PROBLEM - YOU CAN'T USE PREPROCESSOR IFS LIKE THIS IN C#
#if PREDICTION_ERROR_CHECK_LEVEL > 0 
...
void _CheckV( int tick, char const *ctx, const Vector3 &vel )
{
    ...
    void CGameMovement::DiffPrint( char const *fmt, ... )
{
...
#else
    static void DiffPrint(bool bServer, int nCommandNumber, char fmt/*, ...*/ )
    {
        // Nothing
    }
    static void StartCommand(bool bServer, int nCommandNumber)
    ...
    //#define CheckV( tick, ctx, vel ) UNCLEAR WHAT THIS IS FOR
}


//WE'RE MISSING A BUNCH OF CLASS DEFINITIONS - HERE ARE SOME EMPTY ONES FOR NOW
...
public class CBaseHandle
{

}

public class CBaseTrace
{
